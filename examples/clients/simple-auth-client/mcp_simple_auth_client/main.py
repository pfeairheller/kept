...
from kept.mcp.client.essr import essr_client
    ...
    async def connect(self):
        ...
        try:
            ...
            if self.transport_type == "sse":
                ...
                with existing.existingHab(name=self.name, alias=self.alias) as (
                    hby,
                    hab,
                ):
                    crypt_signer = CryptSigner(hby, hab, encryption_target=self.target)
...
async def main(name, alias, target):
    ...
    client = SimpleAuthClient(
        server_url, transport_type, name=name, alias=alias, target=target
    )
